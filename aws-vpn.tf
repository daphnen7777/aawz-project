...
resource "aws_customer_gateway" "cgw-for-azrm-0" {
    bgp_asn = 65515
...
resource "aws_customer_gateway" "cgw-for-azrm-1" {
    bgp_asn = 65515
...
resource "aws_vpn_connection" "aws-azrm-vpn-0" {
    vpn_gateway_id = aws_vpn_gateway.vgw-for-azrm.id
    customer_gateway_id = aws_customer_gateway.cgw-for-azrm-0.id
    type = "ipsec.1"
...
resource "aws_vpn_connection" "aws-azrm-vpn-1" {
    vpn_gateway_id = aws_vpn_gateway.vgw-for-azrm.id
    customer_gateway_id = aws_customer_gateway.cgw-for-azrm-1.id
    type = "ipsec.1"
    static_routes_only = false
    local_ipv4_network_cidr = "0.0.0.0/0"
    remote_ipv4_network_cidr = "0.0.0.0/0"
